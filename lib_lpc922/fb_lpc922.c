...
void X1_int(void) __interrupt (2)		// Flanke des Startbits laest int1 aus
{
	...
	TH1=128;//128;			// autoreload auf 35µs setzen (gilt fuer sprung von 2 auf 3)
...
void T1_int(void) __interrupt (3) 	// Timer 1 Interrupt
{
	...
	case 4:	//	Timeout, d.h. Telegramm-Ende
		if (auto_ack && telpos>4) {		// wenn ACK gesendet werden soll und Telegramm zumindest 5 Bytes hat
			...
			TL1=0x42;
			TR1=1;
			its_me=transparency;			// indiziert, ob dieses Geraet adressiert wurde, bei transparecy=1 immer
			if(telegramm[5]&0x80) {
...
//sparen						connected_timeout=0;// den timeout rücksetzen
		...
		// Multicast, wenn Zieladresse Gruppenadresse ist
			...
			if (gapos != 0xFF)
			{
				...
				// Schleife ueber alle Eintraege in der Ass-Table, denn es könnten mehrere Objekte
